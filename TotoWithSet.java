...
public class TotoWithSet extends readResFromFile {
...
//		printRes(suppose);
        double officialRsStd = stdOfRow(this.officialRes);
        System.out.print(this.officialRes);
        System.out.printf(" std: %.2f%n", optimizedStdOfRow(this.officialRes));
...
//		System.out.println();
        ...
        List<List<Integer>> res = generateByMatches(this.officialRes, this.matchedSums, this.std);
        printRes(res);
        TestStd(res);
        ...
        System.out.println();
        print2DRes(res, this.officialRes);
        writer(res, this.officialRes, duration, this.matchedSums, std);
    }

    static void printRes(List<List<Integer>> suppose) {
        for (List<Integer> row : suppose) {
            for (int el : row) {
                System.out.print(el + " ");
            }
    ...
    static List<List<Integer>> generateByStd(double std) {
        int row = 0;
        double tmpStd = Double.MIN_VALUE;
        List<Integer> tmpRow;
    ...
    private static List<List<Integer>> generateByMatches(List<Integer> officialRes,
    ...
    static int checkMatchesByRow(List<Integer> row, List<Integer> officialResult) {
        int matches = 0;

        for (Integer elOfRow : row) {
            if (officialResult.contains(elOfRow)) {
                matches++;
            }
        }
        return matches;
    ...
    public static double stdOfRow(List<Integer> row) {
    ...
    public static double optimizedStdOfRow(List<Integer> row) {
        double sum = 0;
        double squaredSum = 0;
        for (int el : row) {
            sum += el;
    ...
    private static void TestStd(List<List<Integer>> listOfRows) {
        System.out.print("Stds: ");
        for (int i = 0; i < listOfRows.size(); i++) {
            if (i < 2) {
                System.out.print(stdOfRow(listOfRows.get(i)) + " | ");
            } else System.out.print(stdOfRow(listOfRows.get(i)));
        }
    }
    ...
    static void print2DRes(List<List<Integer>> res, List<Integer> officialRes) {
    ...
    static void writer(List<List<Integer>> res,
        ...
        double stdOffRes =(optimizedStdOfRow(officialResult));
        String stdOfResByStr = String.format("%.2f", stdOffRes);
        writer.write("\n" + "Official result: " + officialResult + " - " + "Std: " + stdOfResByStr + "\n");
