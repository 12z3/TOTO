...
public class Checks {
	public static void main(String[] args) {
		forNewDraw(Manipulate.getThreeOfficialDB());
	}
	 ...
	 * </li>
	 */
	static boolean isNewDrawOnSiteOld(List<String> siteData) {
		String lastWrote;
	...
	static boolean isNewDrawOnSite(List<String> siteData) {
		LocalDate lastOfficialDrawDate = Manipulate.getLastOfficialDrawDate(siteData);
		LocalDate lastDateFromArch = Manipulate.getLastDateFromFile();
		return (lastOfficialDrawDate.isAfter(lastDateFromArch));
	}
	...
	static boolean[] urlArr(boolean[] checkRes, List<String> siteData) {
		...
		try {
			...
			System.out.println("404: Няма Интеренет Връзка май....");
...
//                WebData.getWebIdx("/Users/blagojnikolov/Desktop/@tmp/bgTotoFromSite.txt"));
		...
		int lastDrawIdxFromSiteData = Manipulate.getLastDrawIdxFromSiteData(siteData).get(1);
...
//                WebData.getWebIdx("/Users/blagojnikolov/Desktop/@tmp/fromSite.txt"));
	 ...
	 */
	static String yourSupposesOld(List<String> siteData) {
		String error = "Error";
		String data = "";
	...
	static String yourSupposes(List<String> siteData) {
		int webIdx = -1;
		List<Integer> webData = new ArrayList<>();
		Map<Integer, List<Integer>> maxIdxDrawDigits = getIdxDigits_MapFromLastDateOfOfficialDraws(siteData);

		for (Map.Entry<Integer, List<Integer>> entry : maxIdxDrawDigits.entrySet()) {
			webIdx = entry.getKey();
			webData = entry.getValue();
		}
		checksByIndexes(webData, webIdx);
		return "No Error";
	}
	...
	/**
	 * Връща Дата от елемент на даден ред
	 */
	static LocalDate getDateOfThisString(String line) {
		String dateTmp = line.trim().split("-")[1];
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd MMM yyyy", Locale.ENGLISH);
		return LocalDate.parse(dateTmp, formatter);
	}

	/**
	 * Взима масива от siteData:
	 * <p>Извлича индекса и датата на всяко теглене.
	 * Връща последната дата и индекс.</p>
	 **/
	static Map<Integer, List<Integer>> getIdxDigits_MapFromLastDateOfOfficialDraws(List<String> siteData) {
		int idx = Integer.parseInt(siteData.getFirst().trim().split("-")[0]);
		LocalDate date = getDateOfThisString(siteData.getFirst());
		List<Integer> digits = List.of();
		try {
			for (String line : siteData) {
				int idxTmp = Integer.parseInt(line.trim().split("-")[0]);
				LocalDate dateTmp = getDateOfThisString(line);
				if (dateTmp.isAfter(date)) {
					idx = idxTmp;
					digits = Manipulate.parseToInt(line.replace(", ", " ").split(" ")[2]);
				}
			}
			Map<Integer, List<Integer>> idxDrawNumbers = new HashMap<>();
			idxDrawNumbers.put(idx, digits);
			return idxDrawNumbers;
		} catch (NumberFormatException e) {
			System.out.println("↪︎404: getIdxDigits_MapFromLastDateOfOfficialDraws(): Проблем с данните от siteData. +" +
					" Не мога да извлека idx и LocalDate");
			throw new RuntimeException(e);
		}
	}
